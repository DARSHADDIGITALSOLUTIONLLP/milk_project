...
// ✅ New: Fetch delivered orders for a specific customer (for card view)
router.get(
  "/users/:id/delivered-orders",
  authenticateUser,
  authorizeRole(["admin", "delivery_boy"]),
  deliveryBoyController.getUserDeliveredOrders
);
...
router.get(
  "/users/pending-morning-orders",
  authenticateUser,
  authorizeRole(["admin", "delivery_boy"]),
  deliveryBoyController.morningPendingOrders
);

// api for the fetching the evening pending delivery orders
router.get(
  "/users/pending-evening-orders",
  authenticateUser,
  authorizeRole(["admin", "delivery_boy"]),
  deliveryBoyController.eveningPendingOrders
);
...
router.post(
  ...
  authenticateUser,
  authorizeRole(["admin", "delivery_boy"]),
  deliveryBoyController.addDeliveryRecord
);
